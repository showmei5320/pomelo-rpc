...
var Gateway = function(opts) {
  ...
  this.activeRequests = 0;  // request count
  ...
  this.acceptor = this.acceptorFactory.create(opts, (tracer, msg, cb) => {
    if (!this.acceptingRequests) {
      return cb(new Error("Server is shutting down"));  // 拒绝新请求
    }
    this.activeRequests++;
    const wrapCallback = (err, result) => {
      this.activeRequests--;
      cb(err, result);
    };
    try {
      dispatcher.route(tracer, msg, wrapCallback);
    } catch (error) {
      this.activeRequests--;
      cb(error);
    }
...
pro.stop = function(callback) {
  ...
  if (this.activeRequests > 0) {
    ...
    setTimeout(() => this.stop(callback), 1000);  // 延遲關機檢查
    return;
  }
  this.acceptor.close(() => {
