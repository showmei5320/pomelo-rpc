...
var MailBox = function(server, opts) {
  ...
  this.closed = false;
  this.isShuttingDown = false; // 新增：服務器是否正在關閉的標誌
  this.opts = opts;
...
MailBox.prototype.close = function() {
  ...
  this.closed = true;
  this.connected = false;
  this.isShuttingDown = true; // 標記為關閉狀態
  if (this._interval) {
...
MailBox.prototype.send = function(tracer, msg, opts, cb) {
  ...
  if (this.closed || this.isShuttingDown) { // 檢查是否正在關閉
    tracer && tracer.error('client', __filename, 'send', 'mailbox has already closed or is shutting down');
    cb(tracer, new Error(this.serverId + ' mqtt-mailbox has already closed or is shutting down ' + this.id));
    return;
  }
