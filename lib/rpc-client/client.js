...
pro.rpcInvoke = function(serverId, msg, cb) {
  ...
  // 改動部分：檢查是否服務器正在關閉
  if (msg.serverShuttingDown) {
    tracer && tracer.error('client', __filename, 'rpcInvoke', 'server is shutting down');
    logger.error('[pomelo-rpc] server is shutting down');
    cb(new Error('[pomelo-rpc] server is shutting down'));
    return;
  }
